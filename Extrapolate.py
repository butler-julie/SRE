...
def sequential_extrapolate(R, y_train, num_points, seq=2,\
    ...
    assert isinstance(isErrorAnalysis, 'boolean')
    ...
    if isErrorAnalysis:
        EA = ErrorAnalysis()
        print("The MSE score between the predicted and true data \
            is", EA.mse(np.asarray(y_test), np.asarray(y_true)))
        print("The R2 score between the predicted and true data \
            is", EA.r2(np.asarray(y_test), np.asarray(y_true)))
...
##############################
def sequential_column_extrapolate (R, formattedData, num_new_cols):
    """
        Inputs:
    ...
    for i in range(len(formattedData)):
        ...
        row = formattedData[i]
        X_train, y_train = format_sequential_data(row)
        # Using the current row as training data, extrapolate until the correct
        # length is reached (given by num_new_cols)
        R.fit(X_train, y_train)
        weights.append(R.get_parameters())
        new_row = sequential_extrapolate(R, y_train, num_new_cols)
