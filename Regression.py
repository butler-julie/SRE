...
class Regression():
    ...
    def predict(self, points):
        ...
        if not isFit:
            ...
            y_hat = []
            for point in points:
                y_hat.append(point@self.weights+self.y_offset)
            y_hat = y_hat
    ...
    def get_normalization_parameters (self):
        ...
        print()
        return self.y_offset, self.X_offset, self.X_scale     
...
class LiR(Regression):
    ...
    def fit(self, X, y):
        ...
        self.weights = (np.linalg.inv(np.transpose(X)@X)@np.transpose(X)@y).flatten()
...
class RR(Regression):
    ...
    def fit(self, X, y):
        ...
        # Find the trained weights using the analytical, closed-form expression       
        self.weights = (np.linalg.inv(np.transpose(X)@X -\
            self.alpha*np.eye(len(X)))@np.transpose(X)@y).flatten()
        # Update the model to know that it has been fit
        self.isFit = True
...
class KRR (Regression):
    ...
    # INIT
    def __init__ (self, params, kernel_func, isModified = True):
        """ 
            Inputs:
    ...
    def fit (self, X, y):
        ...
        for i in range(len(X)):
            ...
            for j in range(len(X)):
                row.append(self.kernel_func(X[i], X[j],power, c0, gamma))
            kernel.append(row)
        ...
        if not isFit:
            ...
            for x in range(len(points)):
                ...
                for i in range(len(kernel)):
                    pred += self.weights[i]*self.kernel_func(self.X_train[i],points[x],self.params)
                y_hat.append(pred)
            return y_hat   